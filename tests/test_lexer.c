...
void test_tokenize_number() {
    ...
    TEST_ASSERT_EQ(tokens->type, NUMBER, "First token should be NUMBER");
...
void test_tokenize_var_keyword() {
    struct token *tokens = tokinize("var");
    TEST_ASSERT(tokens != NULL, "Tokens should not be NULL");
    TEST_ASSERT_EQ(tokens->type, KEYWORD_VAR, "First token should be KEYWORD_VAR");
}
...
void test_tokenize_assignment() {
    ...
    TEST_ASSERT(tokens != NULL, "Tokens should not be NULL");
    TEST_ASSERT_EQ(tokens->type, IDENTIFYER, "First token should be IDENTIFIER");
    TEST_ASSERT_EQ(tokens->next->type, TOKEN_ASSIGN, "Second token should be TOKEN_ASSIGN");
}
